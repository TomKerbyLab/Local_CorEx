...
def load_models(ae_ckpt: str, classifier_ckpt: str, ae_conf: dict, clf_conf: dict) -> Any:
    """
    Load an autoencoder model whose encoder is initialized from a pre-trained classifier.

    Parameters:
    - ae_ckpt (str): Autoencoder checkpoint path.
    - classifier_ckpt (str): Classifier checkpoint path.
    - ae_conf (dict): Configuration dictionary for the autoencoder.
    - clf_conf (dict): Configuration dictionary for the classifier.

    Returns:
    - The loaded autoencoder model and classifier.
    """
    from mnist_classifier.model import Autoencoder, MLPClassifier
...
def compute_cluster_accuracies(clf, loader, device, indexes, verbose=False, return_probs=True):
    ...
    for i in range(len(indexes)):
        base_accuracies.append(
            (100 * torch.sum(pred_label[indexes[i]] == labels_tensor[indexes[i]]) / pred_label[indexes[i]].size(0)).item()
        )
        if verbose:
            print(
                i,
...
def prune_resnet_fc_layer(model, node_indexes):
    ...
    Args:
        ...
        node_indexes: Iterable of node indices to prune.

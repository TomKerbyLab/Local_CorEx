...
def plot_perturved_accuracy(
    ...
    baseline_logits = clf(torch.tensor(inputs, dtype=torch.float32)).detach()
    baseline_probs = torch.nn.functional.softmax(baseline_logits, dim=1)
    baseline_pred = baseline_probs.max(1).indices.numpy()
    ...
    altered_logits = clf_clone(torch.tensor(inputs, dtype=torch.float32)).detach()
    altered_probs = torch.nn.functional.softmax(altered_logits, dim=1)
    altered_pred = altered_probs.max(1).indices.numpy()
    ...
    ax = temp_df.drop(["Factor #", "# dropped"], axis=1).plot(
        kind="bar",
        figsize=(10, 7),
        width=0.8,
        colormap="tab20",
...
def plot_logit_effects(ave_diff_prob, class_names, bottom_vals=3, top_vals=10, save_path: str | None = None):
    ...
    top_n_indices = top_n_indices.flip(0)
    values = np.concatenate((bottom_n_values.cpu().numpy(), top_n_values.cpu().numpy()))
    indices = np.concatenate((bottom_n_indices.cpu().numpy(), top_n_indices.cpu().numpy()))
    ...
    padding = max(0.02, range_val * 0.1)
    ...
    plt.axvline(x=bottom_vals - 0.5, color="black", linewidth=1.5)
